...
public class SideNavigationViewController: UIViewController, UIGestureRecognizerDelegate {
	...
	*/
	public func resetView() {
		...
		print(self.sideView.position)
		MaterialAnimation.animationDisabled {
			self.sideView.width = self.sideViewControllerWidth
			self.sideView.position = CGPointMake((self.opened ? 1 : -1) * self.sideViewControllerWidth / 2, 0)
		}
...
//		sideView.autoresizesSubviews = true
	...
	internal func handlePanGesture(recognizer: UIPanGestureRecognizer) {
		...
		case .Changed:
			...
			let v: CGFloat = self.sideViewControllerWidth / 2
			MaterialAnimation.animationDisabled {
				self.sideView.position.x = self.originalPosition.x + translation.x > v ? v : self.originalPosition.x + translation.x
			}
		case .Ended:
